    ...
    return (
      <div className="bg-vm-blue">
          {props.content.dataList.map((section) => (
            <div id={section.fields.id} className="max-w-7xl mx-auto py-12 px-4 divide-y divide-gray-200 sm:px-6 lg:py-16 lg:px-8">
                ...
                <dl className="divide-y divide-white">
                  {section.fields.list.map((point, i) => (
                    <div key={point.sys.id} className="pt-6 pb-8 md:grid md:grid-cols-12 md:gap-8">
                      <dt className="text-base font-medium text-white md:col-span-5"><a className="hover:underline" href={point.fields.url}>{point.fields.title}</a></dt>
